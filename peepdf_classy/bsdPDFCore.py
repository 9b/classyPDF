...
monitorizedActions = ['/JS','/JavaScript','/Launch','/SubmitForm','/ImportData','/Encrypt']
monitorizedElements = ['/JBIG2Decode','/EmbeddedFiles','/EmbeddedFile','getPageNthWord','arguments.callee','/U3D','/PRC','/Flash','/ASCIIHexDecode','/ASCII85Decode','/LZWDecode','/FlateDecode','/RunLengthDecode','/CCITTFaxDecode','/DCTDecode','/JPXDecode','/Crypt']
...
class PDFDictionary (PDFObject):
    def __init__(self, rawContent = '', elements = {}, rawNames = {}) :
        ...
        self.references = []
        self.suspiciousEvents = [] #bsdixon
        self.suspiciousActions = [] #bsdixon
        self.suspiciousElements = [] #bsdixon
        ret = self.update()
        if ret[0] == -1:
            if isForceMode:
                self.addError(ret[1])
    ...
    def setSuspiciousEvents(self,event): #bsdixon
        self.suspiciousEvents.append(event) #bsdixon
         
    def setSuspiciousActions(self,action): #bsdixon
          self.suspiciousActions.append(action) #bsdixon
         
    def setSuspiciousElements(self,element): #bsdixon
        self.suspiciousElements.append(element) #bsdixon
        ...
        for event in monitorizedEvents:
            if value.find(event) != -1:
                if self.suspiciousEvents.has_key(event):
                    ...
                    pdfObject.setSuspiciousEvents(event) #bsdixon
        for action in monitorizedActions:
            if value.find(action) != -1:                    
                ...
                if self.suspiciousActions.has_key(action):
                    ...
                    pdfObject.setSuspiciousActions(action) #bsdixon
        for element in monitorizedElements:
            if value.find(element) != -1:
                if self.suspiciousElements.has_key(element):
                    ...
                    pdfObject.setSuspiciousElements(element) #bsdixon
